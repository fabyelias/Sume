...
type Novedad = { categoria: string; texto: string; foto: boolean; hora: string };
...
export function PMChecklist({ base, movilFisico, nombreParamedico }: { base: Base; movilFisico: string; nombreParamedico: string }) {
  ...
  const [nuevaNovedad, setNuevaNovedad] = useState({ categoria: "mecanica", texto: "", foto: false });
  ...
  const confirmarCierre = async () => {
    ...
    cierresActuales[nombreParamedico] = { firmado: true, hora, movil: movilFisico, base: base.label, km, novedades: novedades.length };
  ...
  return (
    <div className="space-y-7">
      ...
      <button
        onClick={() => puedeEnviar && setEnviado(true)}
      ...
      {enviado && (
        <section className="space-y-3">
          ...
          <p className="text-sm text-slate-500">Si ocurre algo durante la guardia, reportalo acá. Se refleja en el panel del Jefe y del Mecánico.</p>
          {novedades.length > 0 && (
                ...
                <div key={idx} className={`${card} p-3.5`}>
                  <div className="flex items-center justify-between mb-1">
                    <span
                      ...
                      style={{
                        background: n.categoria === "mecanica" ? `${R}14` : n.categoria === "medicacion" ? "#D9770614" : `${A}14`,
                        color: n.categoria === "mecanica" ? R : n.categoria === "medicacion" ? "#D97706" : A,
                      }}
                    >
                      {n.categoria === "mecanica" ? "Mecánica" : n.categoria === "medicacion" ? "Medicación / insumos" : "Otro"}
                    </span>
                    <span className="text-[11px] text-slate-400">{n.hora}</span>
                  </div>
          ...
          <div className={`${card} p-4 space-y-2`}>
            ...
            <button
              onClick={() => {
                ...
                setNovedades((p) => [{ ...nuevaNovedad, hora: new Date().toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" }) }, ...p]);
                setNuevaNovedad({ categoria: "mecanica", texto: "", foto: false });
              }}
              disabled={!nuevaNovedad.texto.trim()}
            ...
            >
              <PlusCircle size={16} /> Reportar novedad
            </button>
          </div>
        </section>
      )}
          ...
          {!mostrarCierre ? (
            ...
            <div className={`${card} p-5 space-y-4`}>
              <p className="text-sm text-slate-600">
                Al firmar, confirmás que el móvil queda entregado en las condiciones reportadas, incluyendo {novedades.length > 0 ? `${novedades.length} novedad(es)` : "sin novedades adicionales"}.
              </p>
              <div>
